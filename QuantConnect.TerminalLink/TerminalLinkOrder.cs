...
{
    public class TerminalLinkOrder
    {
        ...
        public TerminalLinkOrder(int sequence)
        {
            Sequence = sequence;
        }
        ...
        public bool IsLeanOrder =>
            HasValue(SubType.Order, TerminalLinkNames.EMSXReferenceOrderIdResponse) && HasValue(SubType.Route, TerminalLinkNames.EMSXReferenceRouteId);

        // Use the route for the status, as the parent can report back being partially filled, but the route is cancelled.
        public OrderStatus Status => ConvertOrderStatus(GetString(SubType.Route, TerminalLinkNames.EMSXStatus));

        public int Amount => GetInt(SubType.Route, TerminalLinkNames.EMSXAmount, false);

        public int Filled => GetInt(SubType.Route, TerminalLinkNames.EMSXFilled, false);
        ...
        private T GetDefault<T>(SubType subType, Name name, bool allowDefault)
        {
            ...
            if (Log.DebuggingEnabled)
            {
                ...
                Log.Debug($"TerminalLinkOrder.GetDefault(): Route [{string.Join(",", route)}] -------- Order [{string.Join(",", order)}]");
            }
