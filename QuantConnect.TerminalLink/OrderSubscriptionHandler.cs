...
namespace QuantConnect.TerminalLink
{
    public class OrderSubscriptionHandler : IMessageHandler
    {
        ...
        private readonly TerminalLinkOrders _orders;
        ...
        public OrderSubscriptionHandler(TerminalLinkBrokerage brokerage, IOrderProvider orderProvider, TerminalLinkOrders orders)
        {
        ...
        public void ProcessMessage(Message message)
        {
            ...
            var subTypeStr = message.HasElement(TerminalLinkNames.MessageSubType) ? message.GetElementAsString(TerminalLinkNames.MessageSubType) : null;
            ...
            if (msgType.Equals(TerminalLinkNames.SubscriptionStarted))
            {
                Log.Trace($"OrderSubscriptionHandler.ProcessMessage(type={subType}): Subscription started");
            }
            else if (msgType.Equals(TerminalLinkNames.SubscriptionStreamsActivated))
            {
                Log.Trace($"OrderSubscriptionHandler.ProcessMessage(type={subType}): Subscription stream activated");
            }
            else if (msgType.Equals(TerminalLinkNames.OrderRouteFields))
            {
        ...
        private void OnOrderUpdate(Message message, SubType subType, int sequence)
        {
            ...
            if (TryGetAndUpdateTerminalLinkOrder(sequence, message, subType, orderId, out var bbOrder) && TryGetOrder(orderId, subType, sequence, out var order))
            {
                EmitOrderEvent(bbOrder, order, subType);
            }
        }
        ...
        private static bool TryGetOurOrderId(Message message, out int orderId)
        {
            orderId = -1;
            string element;
            if (message.HasElement(TerminalLinkNames.EMSXReferenceOrderIdResponse))
            {
                element = message.GetElementAsString(TerminalLinkNames.EMSXReferenceOrderIdResponse);
            }
            else
            {
                return false;
        ...
        private bool TryGetAndUpdateTerminalLinkOrder(int sequence, Message message, SubType subType, int orderId, out TerminalLinkOrder bbOrder, [CallerMemberName] string callerMemberName = null)
        {
            bbOrder = _orders.GetBySequenceNumber(sequence);
            if (bbOrder == null)
            {
                Log.Error($"OrderSubscriptionHandler.TryGetAndUpdateTerminalLinkOrder(ord={orderId},seq={sequence},type={subType}): No existing BB order [caller:{callerMemberName}]");
                return false;
            }
        ...
        private void EmitOrderEvent(TerminalLinkOrder bbOrder, Order order, SubType subType)
        {
            ...
            if (ticket != null)
            {
                newOrderEvent.FillPrice = bbOrder.GetDecimal(SubType.Route, TerminalLinkNames.EMSXAvgPrice, false);

                // The TerminalLink API does not return the individual quantity for each partial fill, but the cumulative filled quantity
        ...
        {
            return (EventStatus) message.GetElement(TerminalLinkNames.EventStatus).GetValue();
        }
    }
}